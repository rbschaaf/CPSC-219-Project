...
public class FinderApp extends Application {
  ...
  */
  ...
  private int[] notMap = {Constants.WALL,Constants.HALL,Constants.ROOM,Constants.START,Constants.DEST,Constants.PATH,Constants.REST,Constants.EL,Constants.STAIR,Constants.COFF};
        ...
        for(int row=0;row<Constants.ROWNUM;row++){
          for(int col=0;col<Constants.COLNUM;col++){
            ...
            if(aFloorPlan.getGrid()[row][col] == aFloorPlan.getRoom(roomNum+1000).getRoomsNumber()){
              ...
              //Door colours
            }else if((aFloorPlan.getGrid()[row][col]+1000) == aFloorPlan.getRoom(roomNum+1000).getRoomsNumber())
      ...
      public Label setNumbers(int roomNumbers){
        ...
        if(roomNumbers<1000){
          if (contains(notMap,roomNumbers) == false){
            rooms.setFont(Font.font("Times New Roman", FontWeight.BOLD, Constants.ROOMNUMBER_FONTSIZE));
